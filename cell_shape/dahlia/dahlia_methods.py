...
def wlt_total(im,wlt='sym2',lvl=5,fraction=0.76,histo=True):
    if histo:
        ...
        cl2=im
    coeffs_trous = pywt.swt2(cl2,wlt,lvl,start_level=0)
    total = np.ones(cl2.shape)
    #Add Gaussian blur
    for elts in coeffs_trous:
        cA,(cH,cV,cD) = elts
        var = getNoiseVar(cA,fraction)        
...
def denoiseStack(path,target_dir,frac=0.75,histo=True):
    ...
    for elt in elements:
        ...
        if histo:
            ...
            cl2=img
        cl2 = wavelet_denoising2(cl2,lvl=3,fraction=frac)
...
def threshold_image(image,method="hysteresis",params=[]):
    """Thresholds an image using a specific method specified by the user."""
    if not method in ["hysteresis","simple"]:
        t = eval("skimage.filters.threshold_"+method)(image)
        out = (image>t).astype(np.uint8)
    elif method=="simple":
        t=params[0]
        out = (image>t).astype(np.uint8)
    elif method=='hysteresis':
        out = hysteresis_thresholding(image,params[0],params[1])
    else:
        raise ValueError('Unknown threshold')
    return out

def arm_from_threshold(mask_h,n_iterations):
    ...
    mask = cv2.morphologyEx(mask_h, cv2.MORPH_OPEN, kernel,iterations=n_iterations)
